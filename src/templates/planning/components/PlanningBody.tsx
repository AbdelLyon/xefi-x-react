...
>({
  ...
  const defaultRenderItem = useCallback((
    ...
    const isStartDate = new Date(item.start_date).toDateString() === (day.date as GenericDate).toDate().toDateString()
    const isEndDate = new Date(item.end_date).toDateString() === (day.date as GenericDate).toDate().toDateString()
  ...
  const defaultRenderCell = useCallback((
    ...
    return (
      ...
      >
        ...
        {/* Items */}
        {cellItems.map((item) => 
          renderItem ? renderItem(item, user, day) : defaultRenderItem(item, user, day)
        )}
    ...
    <div className="space-y-px">
      {Array.from({ length: 10 }, (_, rowIndex) => (
        <div key={rowIndex} className="flex">
          {Array.from({ length: Math.min(periodDays.length, 31) }, (_, colIndex) => (
            <Skeleton
              key={colIndex}
    ...
    <div className={mergeTailwindClasses("overflow-x-auto", className)}>
      ...
      <div className="mt-2 flex justify-end px-4 py-2">
        <p className="text-xs text-foreground-500">
          Utilisateurs visibles: {visibleUsers.length} / {groups.reduce((total, group) => total + group.users.length, 0)}
        </p>
      </div>
    </div>
  )
