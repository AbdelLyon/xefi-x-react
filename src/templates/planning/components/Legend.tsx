...
export const Legend: React.FC<LegendProps> = ({
  ...
  const handleToggle = useCallback((itemId: string, visible: boolean) => {
    config.onToggle?.(itemId, visible)
  }, [config])
  
  const renderLegendItem = useCallback((item: LegendItem, isVisible: boolean = true) => (
    <div
      key={item.id}
  ...
  return (
    <div className={mergeTailwindClasses(
      ...
      <div className="mb-2 flex items-center justify-between">
        ...
        {config.toggleable && (
          <Tooltip content="Gérer la visibilité">
            <Button
              size="sm"
              variant="light"
      ...
      {config.items.length > 0 && (
        <div className="mt-3 border-t border-border pt-2 text-xs text-foreground-500">
          {visibleItems.length} élément{visibleItems.length !== 1 ? 's' : ''} visible{visibleItems.length !== 1 ? 's' : ''}
          {hiddenItems.length > 0 && (
            <span>, {hiddenItems.length} masqué{hiddenItems.length !== 1 ? 's' : ''}</span>
          )}
        </div>
      )}
    </div>
